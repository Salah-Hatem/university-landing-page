...
export function Header({header}: { header: HeaderViewData }) {
    return (
        <HeaderReveal
            className="fixed left-0 top-0 z-99 flex w-full justify-center px-xl desktop:px-3xl pt-2xl tablet:pt-xl">
...
function HeaderLogo({logo}: { logo: HeaderImage }) {
    return (
        <Image
            alt={logo.alt}
            className="h-auto w-47.5 tablet:w-63.25 mobile:w-78.75"
            height={logo.height}
            priority
            src={logo.url}
